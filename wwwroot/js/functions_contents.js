function selectLegend (dataType, e) {
	...
	$('.legendSelect').not('#legend_'+dataType).val("00");	
	...
	for(var i=0; i<totalLength; i++) {
		...
		if(legendRange == 0){		
			if (thisSimilarity >= legendRange) {
				...
				rowNo++;
				addRow += '<tr id="row'+rowNo+'" class="rows_'+dataType+'"><td class=""></td><td class="no">'+rowNo+'</td><td>'+thisRow['title']+'</td><td>'+thisRow['similarity']+'</td><td>'+thisRow['year']+'</td><td><a href=""><img src="/img/seo.png" style="width:30px;"></a></td></tr>';				
			...
			if (thisSimilarity >= legendRange && thisSimilarity < (legendRange+0.1)) {
				...
				rowNo++;
				addRow += '<tr id="row'+rowNo+'" class="rows_'+dataType+'"><td class=""></td><td class="no">'+rowNo+'</td><td>'+thisRow['title']+'</td><td>'+thisRow['similarity']+'</td><td>'+thisRow['year']+'</td><td><img src=""></td></tr>';				
				$('#table_'+dataType+'>tbody').html(addRow);
			}
		}
	}
